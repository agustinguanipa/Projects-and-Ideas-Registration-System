...
function load(page){
	...
	$("#loader").fadeIn('slow');
	$.ajax({
		url:'../../ajax/usuario/listar_usuarios.php',
		data: parametros,
...
$( "#add_usuario" ).validate( {
          ...
          data:
            {
              cedul_usua: function()
              {
                return $('#add_usuario :input[name="cedul_usua"]').val();
              }
            }
        }     
      },
          ...
          data:
            {
              usuar_usua: function()
              {
                return $('#add_usuario :input[name="usuar_usua"]').val();
              }
            }
        }     
      },
    ...
    $.ajax({
      type: "POST",
      url: "../../ajax/usuario/guardar_usuario.php",
      data: formData,
      contentType: false,
      processData: false,
       beforeSend: function(objeto){
...
$('#addUsuarioModal').on('hidden.bs.modal', function(e) {
  $(this).find('#add_usuario')[0].reset();
...
$('#editUsuarioModal').on('hidden.bs.modal', function(e) {
  $(this).find('#edit_usuario')[0].reset();
  $(this).find('.is-valid').removeClass('is-valid');
});
...
$( "#delete_usuario" ).submit(function( event ) {
  var parametros = $(this).serialize();
	$.ajax({
			type: "POST",
			url: "../../ajax/usuario/eliminar_usuario.php",
			data: parametros,
			 beforeSend: function(objeto){
				$("#resultados").html("Enviando...");
			  },
