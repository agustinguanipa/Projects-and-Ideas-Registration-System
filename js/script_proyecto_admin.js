...
$('#editProyectoAdminModal').on('show.bs.modal', function (event) {
...
$( "#edit_proyecto_admin" ).validate( {
    ...
    $.ajax({
      type: "POST",
      url: "../../ajax/proyecto/editar_proyecto_admin.php",
      data: parametros,
       beforeSend: function(objeto){
        $("#resultados").html("Enviando...");
        },
      ...
      $("#resultados").html(datos);
      load(1);
      $('#editProyectoAdminModal').modal('hide');
      }                     
    });
  }
});

$('#editProyectoAdminModal').on('hidden.bs.modal', function(e) {
  $(this).find('#edit_proyecto_admin')[0].reset();
  $(this).find('.is-valid').removeClass('is-valid');
});
...
$('#deleteProyectoAdminModal').on('show.bs.modal', function (event) {
  var button = $(event.relatedTarget) // Button that triggered the modal
  var ident_proy = button.data('ident_proy') 
  $('#delete_id').val(ident_proy)
})

$( "#delete_proyecto_admin" ).submit(function( event ) {
  var parametros = $(this).serialize();
  $.ajax({
      type: "POST",
      url: "../../ajax/proyecto/eliminar_proyecto_admin.php",
      data: parametros,
       beforeSend: function(objeto){
        $("#resultados").html("Enviando...");
        },
      ...
      $("#resultados").html(datos);
      load(1);
      $('#deleteProyectoAdminModal').modal('hide');
      }
  });
